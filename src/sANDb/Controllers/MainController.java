...
public class MainController implements Initializable,Init {
    ...
    @FXML private AnchorPane products, sells, employers,buys,raw;
    @FXML private TableView<Product> productsTable ;
    @FXML private TableView<Product> RawTable ;
    ...
    @FXML public Button seeRecords,day,week,month,total,btn_products, btn_sells, btn_employers,btn_buys,btn_raw;
    ...
    @FXML private JFXButton updateImage,viewHistory,addProd,printProducts,removedProduct,productStats,deleteProduct,updateProduct,
                            ...
                            exBtn,addEmployerButton,printBuy,printBuys,newBuyBtn,buyStatBtn,updateRaw,rawStats,deleteRaw;
    ...
    public void getEmployer(Employer employer) {
        ...
        if(employer.getAdmin() == 0)
        {
            ...
            if(employer.getrawPrivs() == 0){
                btn_raw.setDisable(true);
    ...
    private void search(String tableName)
    {
        ...
        if (keyword.trim().equals("")) {
       ...
       else if(tableName.trim().equalsIgnoreCase("raw")){
           ...
           if (keyword.trim().equals("")) {
               ...
               ObservableList<Product> filteredData = FXCollections.observableArrayList();
               data.stream().filter((raw) -> (raw.getName().toLowerCase().contains(keyword.toLowerCase()))).forEachOrdered((raw) -> {
                   filteredData.add(raw);
                });
               RawTable.setItems(filteredData);
           }           
        }
    }    
    ...
    private void showEmployer(String username)
    {
        ...
        if(choosen.getProdPrivs() == 1) prodManager.setImage(new Image(
                    ClassLoader.class.getResourceAsStream("/sANDb/images/granted.png"),
                    25, 25, true, true));
        else
            prodManager.setImage(new Image(
                    ClassLoader.class.getResourceAsStream("/sANDb/images/notgranted.png"),
                    25, 25, true, true));    
    ...
    private void getAllSells(String selectedDate)
    {
        ...
        String query_2 = "SELECT * FROM sell INNER JOIN product ON sell.prod_id = product.prod_id INNER JOIN user ON sell.user_id = user.user_id  WHERE  user.customer = ? AND date(sell_date) = ? ORDER BY time(sell_date) ASC ";
        ...
        try {
            ...
            st2 = con.prepareStatement(query_2);
            st.setString(1,selectedDate);
            st2.setInt(1,1);
            st2.setString(2,selectedDate);
           
            rs = st.executeQuery();
            rs2 = st2.executeQuery();
//            while(rs2.next()){
//                 System.err.println(rs2.getString("username"));
//            }
            while (rs.next()) {
                Sell sell = new Sell();
                ...
                if(rs2.next()){
                sell.setCustomer(rs2.getString("username"));
                }
    ...
    public void getSellStats(String selectedDate, String type){
        ...
        if(selectedDate.equals("")){
            ...
            query = "SELECT count(*), SUM(sell_quantity) FROM sell WHERE date(sell_date) = ? ";
        }
        ...
        try {
           ...
           // int priceSum = 0;
            ...
            //revSum.setText(String.valueOf(priceSum) + " شيكل");
    ...
    public void getBuyStats(String selectedDate, String type){
        ...
        if(selectedDate.equals("")){
            ...
            query = "SELECT count(*), SUM(buy_qte) FROM buy WHERE date(buy_date) = ? ";
        }
        ...
        try {
            ...
            //buyDaySum.setText(String.valueOf(priceSum) + " شيكل");
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        sellTotalCol.setCellValueFactory(new PropertyValueFactory<>("customer"));
